...
class AmmoPickup(pygame.sprite.Sprite):
    def __init__(self,x,y):
        super().__init__()
        self.image=bullet_icon(16)
        self.rect=self.image.get_rect(center=(x,y))

    def apply(self,player):
        player.ammo=min(player.ammo+MAG_CAPACITY//2,MAG_CAPACITY)
        self.kill()
...
class Player(Entity):
    def __init__(self,x,y):
        ...
        self.health=MAX_HEALTH;self.ammo=MAG_CAPACITY
...
def main():
    ...
    while True:
        ...
        if state=="loading":
            ...
            g=game;lvl,ply,en,blt,pick=g["lvl"],g["ply"],g["enem"],g["blt"],g["pick"]
            ...
            if hits:
                g["kills"]+=sum(len(v)for v in hits.values())
                for grp in hits.values():
                    for dead in grp:
                        r=random.random()
                        if r<HEALTH_DROP_CHANCE:
                            pick.add(HealthPickup(dead.rect.centerx,dead.rect.centery))
                        elif r<HEALTH_DROP_CHANCE+AMMO_DROP_CHANCE:
                            pick.add(AmmoPickup(dead.rect.centerx,dead.rect.centery))
            for enemy in en:
            ...
            for p in pygame.sprite.spritecollide(ply,pick,False):
                p.apply(ply)
            ...
            for s in pick:screen.blit(s.image,(s.rect.x-cam_x,s.rect.y))
